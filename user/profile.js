...
class Profile {
  ...
  async trainModel(force) {
    ...
    if (
      ...
      for (const [topic, val] of this.topics.entries()) {
        if (!val.get("isTrained") && !val.get("isInTraining")) {
          ...
          dataImportPromise = dataImportPromise
            .then(() => {
              return this.dataset.importDatasetItems(bucketName, fileName);
            })
            .catch((error) => {
              console.log(error);
            });
      ...
      dataImportPromise
        .then(() => {
          ...
          return this.modelInTraining.getTrainingPromise();
        })
        .then(() => {
          // Set isTrained to true when topic finished training.
          this.topics.forEach(async (val, topic) => {
            if (val.get("isInTraining")) {
        ...
        .catch((error) => {
          ...
          // If model training fails, topic is not be trained.
          this.topics.forEach(async (val, topic) => {
            if (val.get("isInTraining")) {
              this.topics.get(topic).set("isInTraining", false);
              this.topics.get(topic).set("isTrained", false);
            }
          });
